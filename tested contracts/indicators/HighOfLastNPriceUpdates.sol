...
contract HighOfLastNPriceUpdates is IIndicator {
    ...
    */
    function update(uint latestPrice) public override {
        ...
        {
            high = (_tradingBotStates[msg.sender].history[_tradingBotStates[msg.sender].history.length - i - 1] > high) ? _tradingBotStates[msg.sender].history[_tradingBotStates[msg.sender].history.length - i - 1] : high;
        }
