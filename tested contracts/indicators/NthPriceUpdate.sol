...
contract NthPriceUpdate is IIndicator {
    ...
    */
    function update(uint latestPrice) public override {
        ...
        if (_tradingBotStates[msg.sender].priceHistory.length < uint256(_tradingBotStates[msg.sender].N))
        {
            _tradingBotStates[msg.sender].indicatorHistory.push(0);
        }
        else
        {
            uint index = _tradingBotStates[msg.sender].priceHistory.length.sub(uint256(_tradingBotStates[msg.sender].N));
            uint value = _tradingBotStates[msg.sender].priceHistory[index];
    ...
    */
    function getValue(address tradingBotAddress) public view override returns (uint[] memory) {
        ...
        if (_tradingBotStates[tradingBotAddress].indicatorHistory.length > 0)
        {
            temp[0] = _tradingBotStates[tradingBotAddress].indicatorHistory[_tradingBotStates[tradingBotAddress].indicatorHistory.length - 1];
        }
        else
        {
            temp[0] = 0;
