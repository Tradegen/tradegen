...
contract LowOfLastNPriceUpdates is IIndicator {
    ...
    */
    function update(uint index, uint latestPrice) public override {
        ...
        uint low = (length == 0) ? 0 : _tradingBotStates[msg.sender][index].history[_tradingBotStates[msg.sender][index].history.length - 1];

        for (uint i = 0; i < length; i++)
        {
            low = (_tradingBotStates[msg.sender][index].history[_tradingBotStates[msg.sender][index].history.length - i - 1] < low) ? _tradingBotStates[msg.sender][index].history[_tradingBotStates[msg.sender][index].history.length - i - 1] : low;
        }

        _tradingBotStates[msg.sender][index].currentValue = uint120(low);
    }   
