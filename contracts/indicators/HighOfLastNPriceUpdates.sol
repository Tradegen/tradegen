...
contract HighOfLastNPriceUpdates is IIndicator, Ownable {
    ...
    function update(address tradingBotAddress, uint latestPrice) public override {
        ...
        {
            high = (_tradingBotStates[tradingBotAddress].history[length - i - 1] > high) ? _tradingBotStates[tradingBotAddress].history[length - i - 1] : high;
        }

        _tradingBotStates[tradingBotAddress].currentValue = uint128(high);
    }   
