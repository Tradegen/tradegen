...
contract NFTPool is INFTPool, ISellable {
    ...
    */
    function getTokenBalancePerClass(address user) external view override returns(uint, uint, uint, uint) {
        require(user != address(0), "NFTPool: Invalid user address.");
    ...
    */
    function getAssetValue(address asset, address assetHandlerAddress) public view override returns (uint) {
        require(asset != address(0), "NFTPool: Invalid asset address.");
        require(assetHandlerAddress != address(0), "NFTPool: Invalid asset handler address.");
    ...
    */
    function getUSDBalance(address user) public view override returns (uint) {
        require(user != address(0), "NFTPool: Invalid address.");
    ...
    */
    function deposit(uint numberOfPoolTokens) public override {
        require(numberOfPoolTokens > 0 &&
                ...
                "NFTPool: Quantity out of bounds.");
    ...
    */
    function withdraw(uint numberOfPoolTokens, uint tokenClass) public override {
        ...
        require(tokenClass > 0 && tokenClass < 5, "NFTPool: Token class must be between 1 and 4.");
        require(numberOfPoolTokens > 0,
                "NFTPool: Withdrawal amount must be greater than 0.");
        require(numberOfPoolTokens <= balanceOf[msg.sender][tokenClass],
                "NFTPool: Not enough tokens.");
    ...
    * @dev Allows a pool to interact with external protocols.
    ...
    */
    function executeTransaction(address to, bytes memory data) public onlyPoolManager {
        require(to != address(0), "NFTPool: Invalid 'to' address.");
        ...
        require(verifier != address(0), "NFTPool: Invalid verifier.");
        
        (bool valid, address receivedAsset) = IVerifier(verifier).verify(address(ADDRESS_RESOLVER), address(this), to, data);
        require(valid, "NFTPool: Invalid transaction.");
        
        (bool success, ) = to.call(data);
        require(success, "NFTPool: Transaction failed to execute.");
    ...
    */
    ...
    /**
    * @notice Initializes the pool's data and distributes tokens by class.
    * @dev Meant to be called once from NFTPoolFactory contract.
    * @dev Token distribution: 5% C1, 10% C2, 20% C3, 65% C4.
    ...
    * @dev Meant to be called from Marketplace contract.
    ...
    */
    ...
    /**
    * @notice Updates the address of the pool's liquidity mining contract.
    */
    ...
    function _withdrawProcessing(address asset, uint portion) internal returns (uint) {
        ...
        if (transactions.length > 0)
        {
            ...
            for (uint i = 0; i < transactions.length; i++)
            {
                ...
                require(success, "NFTPool: Failed to withdraw tokens.");
            }
